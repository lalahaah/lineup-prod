...
export function Sidebar({ className }: SidebarProps) {
  ...
  const [counts, setCounts] = useState({ campaigns: 0, drafts: 0 })

  useEffect(() => {
    fetch('/api/sidebar/counts')
      .then((res) => res.json())
      .then((data) => {
        if (data && typeof data.campaigns === 'number' && typeof data.drafts === 'number') {
          setCounts(data)
        }
      })
      .catch((err) => {
    ...
    <aside className={cn('sidebar select-none', className)}>
      ...
      <nav className="sb-nav">
        {opLinks.map((link) => {
          ...
          const showBadge = typeof link.count === 'number' && link.count > 0
          ...
          return (
            ...
            >
              {link.icon}
              <span>{link.name}</span>
              {showBadge && <span className="count">{link.count}</span>}
            </Link>
          )
        })}
      </nav>
      ...
      >
        ...
        <div className="min-w-0 flex-1">
          <div className="nm truncate font-sans text-sm font-semibold">
            {isPending ? '로그아웃 중...' : '김현우'}
          </div>
